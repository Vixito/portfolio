...
function Loading() {
  ...
  useEffect(() => {
    ...
    const loading = loadingRef.current as any;
    loading.count = 0;
    ...
    const jump = () => {
      ...
      loading.active = true;
      loading.count += 1;
      if (loading.count > 3 && spanHidden) {
        spanHidden.classList.remove("hide");
      }
      lines.timeScale(2);
      gsap.to(loading, {
        keyframes: [
    ...
    arrowLeft?.addEventListener("pointerdown", () => slow());
    arrowRight?.addEventListener("pointerdown", () => fast());
    arrowDown?.addEventListener("pointerdown", () => down());
    spanHidden?.addEventListener("click", () => fall());

    arrowLeft?.addEventListener("pointerup", () => {
      if (!loading.ouch) reset();
    });
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-[#242428] z-50">
      ...
      <style>{`
        ...
        .loading .skate .arm,
        .loading .skate .arm:before,
        .loading .skate .leg,
        .loading .skate .leg:before {
          content: '';
          width: var(--w, 11px);
          height: 4px;
          top: var(--t, 0);
        ...
        .loading .skate .arm.front {
          --r: var(--arm-front);
        }
        .loading .skate .arm.front:before {
          --r: var(--arm-front-end);
        }
        .loading .skate .arm.back {
          --r: var(--arm-back);
        }
        .loading .skate .arm.back:before {
          --r: var(--arm-back-end);
        }
        .loading .skate .leg {
          --w: 11px;
        ...
        .loading .skate .leg.front {
          --r: var(--leg-front);
        }
        .loading .skate .leg.front:before {
          --r: var(--leg-front-end);
        }
        .loading .skate .leg.back {
          --l: 1px;
          --r: var(--leg-back);
        }
        .loading .skate .leg.back:before {
          --r: var(--leg-back-end);
        }
        .loading .skate .board {
          position: absolute;
