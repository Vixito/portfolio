import { useLanguageStore } from "../stores/useLanguageStore";

// Definición de todas las traducciones
export const translations = {
  es: {
    // Navegación
    nav: {
      home: "Inicio",
      studio: "Studio",
      about: "Acerca de",
      work: "Trabajo",
      projects: "Proyectos",
      clients: "Abonadores",
      workExperience: "Experiencia",
      stack: "Stack",
      studies: "Estudios",
      socials: "Redes Sociales",
      store: "Tienda",
      radio: "Radio",
      blog: "Blog",
      settings: "Ajustes",
      language: "Idioma",
      donations: "Donaciones",
      code: "Código",
      repository: "Repositorio",
    },
    // Páginas comunes
    common: {
      loading: "Cargando...",
      error: "Error",
      noContent: "No hay contenido disponible",
      back: "Volver",
      close: "Cerrar",
      save: "Guardar",
      cancel: "Cancelar",
      delete: "Eliminar",
      edit: "Editar",
      create: "Crear",
      update: "Actualizar",
      next: "Siguiente",
      previous: "Anterior",
      page: "Página",
      of: "de",
      selectOption: "Seleccionar una opción",
      optional: "Opcional",
    },
    // Home / Hero
    home: {
      title: "Carlos Andrés Vicioso Lara",
      subtitle: "Ingeniero de Sistemas, Colombia.",
      specialization: "Especializado en el desarrollo backend.",
    },
    // Timeline
    timeline: {
      checkpoint1: {
        year: "2015",
        title: "SQL a los 13 años",
        description: "Descargué XAMPP, exploré PhpMyAdmin y descubrí mi primer lenguaje de máquina ejecutando código SQL.",
      },
      checkpoint2: {
        year: "2017",
        title: "Lenguaje HTML + Entrenamiento de Lógica",
        description: "Aprendí HTML en el colegio, y en internet aprendí sus formatos, también entrené lógica de programación a base de jugar videojuegos.",
      },
      checkpoint3: {
        year: "2019",
        title: "Análisis y desarrollo de software",
        description: "Diagramas de flujos, pseudocódigo, CSS, Java, JavaScript, Python, PHP, Laravel, arquitectura y más.",
      },
      checkpoint4: {
        year: "2020",
        title: "Comienzo en PHP",
        description: "A pesar de la pandemia. Aprendí PHP y Laravel en el mismo instituto del año pasado.",
      },
      checkpoint5: {
        year: "2022",
        title: "Explorando más de la carrera con la Universidad",
        description: "Logré avanzar entrenando con tecnologías de ciberseguridad, cloud, IA, y más.",
      },
      checkpoint6: {
        year: "2024-2025",
        title: "Automatización & AI + Titulación",
        description: "Especialización en inteligencia artificial y automatización + Diplomado en Ciberseguridad + Título de Ingeniería de Sistemas",
      },
      checkpoint7: {
        year: "2026",
        title: "Vixis Studio",
        description: "Marca empresarial establecida. Servicios de desarrollo, automatización y más.",
      },
      close: "Cerrar",
    },
    // About
    about: {
      title: "Sobre Mí",
      description1:
        "Soy ingeniero de sistemas apasionado por la tecnología y la música, mejor conocido como Vixis. Me especializo en el desarrollo backend, en la automatización de procesos e inteligencia artificial.",
      description2:
        "Mi objetivo no es solo crear soluciones seguras, eficientes y escalables, sino también brindar valor a la comunidad y lograr proyectos para mis abonadores. Ayudo a marcas personales, emprendedores y equipos remotos a ahorrar tiempo, reducir errores y escalar operaciones mediante flujos automatizados o programados con distintas herramientas o tecnologías.",
      description3:
        "Me gusta mucho el desarrollo frontend y videojuegos, la ciberseguridad, y la automatización; pero también me encanta el desarrollo en el lado del servidor y para aplicaciones móviles.",
      followSocials: "Sígueme en las redes sociales",
    },
    // Store
    store: {
      title: "Tienda",
      noProducts: "No hay productos disponibles",
      noProductsDescription: "Vuelve pronto para ver mis productos",
      noProductsInCategory: "No hay productos en esta categoría",
      tryAnotherCategory: "Intenta seleccionar otra categoría",
      buy: "Comprar",
      schedule: "Agéndalo",
      request: "Solicitar",
      viewPricing: "Ver Pricing",
      price: "Precio",
      free: "Gratis",
      searchPlaceholder: "Buscar por nombre...",
      allPrices: "Todos",
      description: "Descripción",
      fullDescription: "Descripción Completa",
      scheduleSoon: "Funcionalidad de agendamiento próximamente disponible",
      sendingRequest: "Enviando solicitud...",
      filterByCategory: "Filtrar por categoría:",
      allCategories: "Todas las categorías",
      showingResults: "Mostrando",
      of: "de",
      products: "productos",
      categoryLabel: "Categoría",
      category: {
        programming: "Programación",
        clothing: "Ropa",
        design: "Diseño",
        consulting: "Asesoría",
        course: "Curso",
        investment: "Inversión",
        music: "Música",
        languages: "Idiomas",
      },
      paymentInfo: {
        title: "Información de pago",
        popoverTitle: "Instrucciones de pago",
        popoverDescription: "Haz clic abajo para ver instrucciones detalladas de pago y video tutorial.",
        viewGuide: "Ver guía de pago →",
      },
    },
    // Projects
    projects: {
      title: "Proyectos",
      loading: "Cargando proyectos...",
      noProjects: "No hay proyectos disponibles",
      noProjectsDescription: "Vuelve pronto para ver mis proyectos",
      visit: "Visitar",
      repository: "Repositorio",
      special: "Especial",
    },
    // Clients
    clients: {
      title: "Abonadores",
      loading: "Cargando abonadores...",
      noClients: "No hay abonadores disponibles",
      noClientsDescription: "Vuelve pronto para ver mis abonadores",
      testimonials: "Testimonios",
      noTestimonials: "No hay testimonios disponibles",
      visit: "Visitar",
      viewTestimonial: "Ver testimonio",
      viewLink: "Ir al enlace",
      errorLoadingClients: "Error al cargar abonadores:",
    },
    // Work Experience
    workExperience: {
      title: "Experiencia",
      loading: "Cargando experiencias...",
      noExperiences: "No hay experiencias disponibles",
      noExperiencesDescription:
        "Vuelve pronto para ver mi experiencia profesional",
      noResults: "No hay experiencias de este tipo",
      downloadCV: "Descargar CV",
      position: "Posición",
      company: "Empresa",
      location: "Ubicación",
      period: "Período",
      description: "Descripción",
      responsibilities: "Responsabilidades",
      achievements: "Logros",
      technologies: "Tecnologías",
      evidence: "Evidencias",
      evidenceImage: "Foto",
      evidenceVideo: "Video",
      evidenceDocument: "Documento",
      evidenceLink: "Enlace",
      viewEvidence: "Ver evidencia",
      category: "Tipo",
      allCategories: "Todas",
      categoryLaboral: "Laboral",
      categoryProfesional: "Profesional",
      categoryRelacionada: "Relacionada",
      categoryFreelance: "Freelance",
      categoryPracticas: "Prácticas / Pasantías",
      categoryDocencia: "Docencia",
      categoryInvestigacion: "Investigación",
      categoryVoluntariado: "Voluntariado",
      type: "Tipo",
      status: "Estado",
      current: "Actual",
      past: "Pasado",
      fullTime: "Tiempo Completo",
      partTime: "Medio Tiempo",
      contract: "Contrato",
      freelance: "Freelance",
      present: "Presente",
      assistant: "Asistente Técnico de Sistemas",
      discordMarketer: "Marketer de Discord",
      fullStackDeveloper: "Desarrollador Full-Stack",
      january: "Enero",
      february: "Febrero",
      march: "Marzo",
      april: "Abril",
      may: "Mayo",
      june: "Junio",
      july: "Julio",
      august: "Agosto",
      september: "Septiembre",
      october: "Octubre",
      november: "Noviembre",
      december: "Diciembre",
    },
    // Studies
    studies: {
      title: "Estudios",
      loading: "Cargando estudios...",
      noStudies: "No hay estudios disponibles",
      noStudiesDescription: "Vuelve pronto para ver mis estudios",
      filter: "Filtrar",
      clearFilters: "Limpiar filtros",
      search: "Buscar",
      titlePlaceholder: "Buscar por título...",
      institutionPlaceholder: "Buscar por institución...",
      type: "Tipo",
      status: "Estado",
      all: "Todos",
      degree: "Título Universitario",
      certification: "Certificación",
      course: "Curso",
      completed: "Completado",
      inProgress: "En progreso",
      hasCertificate: "Tiene certificado",
      yes: "Sí",
      no: "No",
      viewCertificate: "Ver certificado",
      institution: "Institución",
      period: "Período",
      sortByPeriod: "Ordenar por período",
      description: "Descripción",
      present: "Presente",
    },
    // Skills & Technologies
    skills: {
      title: "Stack Tecnológico",
      loading: "Cargando tecnologías...",
      noTechnologies: "No hay tecnologías disponibles",
      noTechnologiesDescription: "Vuelve pronto para ver mis tecnologías",
      filter: "Filtrar",
      clearFilters: "Limpiar filtros",
      search: "Buscar",
      namePlaceholder: "Buscar por nombre...",
      category: "Categoría",
      level: "Nivel",
      experience: "Experiencia",
      all: "Todos",
      language: "Lenguaje",
      framework: "Framework",
      database: "Base de Datos",
      tool: "Herramienta",
      cloud: "Cloud",
      instrument: "Instrumento",
      music: "Música",
      other: "Otro",
      beginner: "Principiante",
      intermediate: "Intermedio",
      advanced: "Avanzado",
      expert: "Experto",
      years: "años",
      ofExperience: "de experiencia",
    },
    // Socials
    socials: {
      title: "Redes Sociales",
      loading: "Cargando...",
      noSocials: "No hay redes sociales disponibles",
      noSocialsDescription: "Vuelve pronto para ver mis redes sociales",
      visit: "Visitar",
      description: "Descripción",
      errorLoading: "Error al cargar redes sociales:",
    },
    // Radio
    radio: {
      title: "Radio Vixis",
      viewMore: "Ver más",
      live_home: "En vivo",
      live: "EN VIVO",
      online: "ONLINE",
      offline: "OFFLINE",
      playing: "Reproduciendo",
      paused: "Pausado",
      volume: "Volumen",
      mute: "Silenciar",
      unmute: "Activar sonido",
      play: "Reproducir",
      pause: "Pausar",
      refresh: "Actualizar",
      previous: "Anterior",
      next: "Siguiente",
      loading: "Cargando...",
      offlineTitle: "Radio fuera de línea",
      offlineArtist: "No se puede conectar al servidor",
      liveTitle: "En vivo",
      liveArtist: "Radio Vixis",
      waiting: "Esperando transmisión...",
      unknownTitle: "Título desconocido",
      unknownArtist: "Artista desconocido",
      chat: "Chat",
      events: "Próximos Eventos",
      noEvents: "No hay eventos próximos",
      loadingEvents: "Cargando eventos...",
      chatPlaceholder: "Escribe un mensaje...",
      send: "Enviar",
      username: "Nombre de usuario",
      setUsername: "Establecer nombre",
      usernameRequired: "Por favor ingresa un nombre de usuario",
      messageTooShort: "El mensaje no puede estar vacío",
      messageTooLong: "El mensaje no puede exceder 200 caracteres",
      rateLimit: "Por favor espera antes de enviar otro mensaje",
      sending: "Enviando...",
      noMessages: "No hay mensajes aún",
      loadingMessages: "Cargando mensajes...",
      autoplayBlocked:
        "No se pudo reproducir automáticamente (puede requerir interacción del usuario)",
      ago: "hace",
      minutes: "minutos",
      hours: "horas",
      days: "días",
      now: "ahora",
      usernameTooShort: "El nombre debe tener al menos 2 caracteres",
      usernameInvalid:
        "El nombre solo puede contener letras, números, guiones y guiones bajos",
      bannedWords: "El mensaje contiene palabras no permitidas",
      requestSong: "Pide tu canción",
      songRequestUrlNotConfigured: "URL de Typeform no configurada",
      jingleTitle: "¡Sintonízate!",
      connecting: "Conectando...",
    },
    // Blog
    blog: {
      title: "Blog",
      loading: "Cargando posts...",
      noPosts: "No hay posts disponibles",
      noPostsDescription: "Vuelve pronto para ver mis posts",
      readMore: "Leer más",
      platform: "Plataforma",
      date: "Fecha",
    },
    // Studio
    studio: {
      title: "Vixis Studio",
      description: "Creamos sistemas que piensan y arte que conecta.",
      mission: "Misión",
      vision: "Visión",
      missionDescription:
        "Nuestra misión es crear soluciones innovadoras y eficientes para nuestros abonadores.",
      visionDescription:
        "Nuestra visión es ser un referente y una agencia de desarrollo que crea arte que conecta con las comunidades a través de la automatización.",
      technologies: "Tecnologías:",
      status: "Estado:",
      client: "Abonador:",
      active: "En desarrollo",
      comingSoon: "Próximamente...",
      activeStatus: "Status: Activo",
      highPriority: "Prioridad: Alta",
      socialNetworks: "Redes Sociales",
      socialNetworksSubtitle: "Conecta con nosotros en tus plataformas favoritas",
      instagramDesc: "Contenido visual y proyectos creativos",
      tiktokDesc: "Videos cortos sobre desarrollo y tech",
      whatsappDesc: "Contacto directo y soporte",
      facebookDesc: "Comunidad y actualizaciones",
      telegramDesc: "Canal oficial y anuncios",
      xDesc: "Actualizaciones rápidas y conversaciones",
    },
    // Contratos públicos
    contracts: {
      title: "Contrato de servicios",
      lockedDesc: "Este contrato está protegido. Introduce la contraseña que recibiste junto con el enlace para verlo y firmarlo.",
      passwordPh: "Contraseña del contrato",
      unlock: "Desbloquear contrato",
      unlocking: "Verificando…",
      client: "Cliente",
      company: "Empresa",
      amount: "Importe",
      pendingSig: "Pendiente de firma completa",
      signedBoth: "Completado — firmado por ambas partes",
      signatures: "Firmas",
      provider: "Proveedor",
      signed: "Firmado",
      notSigned: "Sin firmar",
      signNote: "Al firmar aceptas las condiciones descritas arriba. La firma es electrónica (nombre + fecha/hora) y verificada por email.",
      namePh: "Tu nombre completo",
      emailPh: "Tu email (debe coincidir con el del contrato)",
      signBtn: "Firmar contrato",
      signing: "Firmando…",
      otpTitle: "Verificación por email",
      otpDesc: "Te enviamos un código de 6 dígitos a",
      sendCode: "Enviar código",
      sendingCode: "Enviando…",
      newCode: "Pedir otro código",
      codePh: "Código de 6 dígitos",
      verify: "Verificar",
      verifying: "Verificando…",
      verified: "Verificado ✓",
      download: "Descargar PDF del contrato",
      generating: "Generando PDF…",
      needName: "Escribe tu nombre para firmar",
    },
    // Status
    status: {
      title: "Estado Profesional",
      sendRequest: "Enviar una Petición",
      hoursByStatus: "Horas por Estado (Semana)",
      availableHours: "Horas Disponibles (Semana)",
      statusDistribution: "Distribución de Estados (Total)",
      available: "Disponible",
      away: "Ausente",
      busy: "Ocupado",
      availableThisWeek: "Disponible esta semana",
      awayThisWeek: "Ausente esta semana",
      busyThisWeek: "Ocupado esta semana",
      requestForm: "Enviar Petición",
      formDescription: "Completa el formulario para enviar tu petición",
      name: "Nombre",
      namePlaceholder: "Tu nombre completo",
      email: "Email",
      emailPlaceholder: "tu.email@ejemplo.com",
      type: "Tipo de Petición",
      typePlaceholder: "Selecciona el tipo",
      jobOffer: "Oferta de Trabajo",
      collaboration: "Colaboración",
      project: "Proyecto",
      other: "Otro",
      message: "Brief del Proyecto / Mensaje",
      messagePlaceholder: "Describe tu visión...",
      estimatedInvestment: "Inversión Estimada",
      currency: "Divisa",
      currencyCOP: "COP",
      currencyUSD: "USD",
      selectCurrency: "Seleccionar divisa",
      selectRange: "Seleccionar rango",
      encryptionNotice:
        "ENCRIPTACIÓN ACTIVA. TUS DATOS SERÁN PROCESADOS BAJO PROTOCOLOS DE PRIVACIDAD NIVEL A+.",
      successMessage:
        "Tu mensaje se envió exitosamente. Me pondré en contacto contigo pronto.",
      giveTestimonial: "Da un testimonio",
      testimonialUrlNotConfigured: "URL de Typeform no configurada",
      submit: "Enviar",
      sendProposal: "Enviar Propuesta",
      cancel: "Cancelar",
      visionTitle: "TRABAJA CONMIGO",
      visionDescription:
        "¿Tienes un proyecto desafiante, una duda técnica o simplemente quieres expandir tu red? Hablemos.",
      step1Title: "CONTACTO INICIAL",
      step1Description:
        "El contacto se redirigirá a mi correo y analizaré tu mensaje en cuestión de horas.",
      step2Title: "DISCOVERY CALL",
      step2Description:
        "Agendamos una reunión de aproximadamente 15 minutos para alinear objetivos y visión.",
      step3Title: "PROPUESTA TÉCNICA",
      step3Description:
        "Te envío un plan detallado, presupuesto y cronograma real.",
      directEmail: "CORREO DIRECTO",
      callWhatsapp: "WHATSAPP DIRECTO",
      preferVideocall: "¿PREFIERES UNA VIDEOLLAMADA?",
      discoveryCall: "DISCOVERY CALL DE 15 MIN",
      scheduleNow: "AGENDAR AHORA",
      orWriteHere: "O ESCRÍBEME AQUÍ",
      identityName: "IDENTIDAD / NOMBRE",
      linkChannelEmail: "CANAL DE ENLACE / EMAIL",
      mobileFrequencyPhone: "FRECUENCIA MÓVIL / TELÉFONO",
      pesos: "PESOS",
      dollars: "DÓLAR",
      example: "Ejemplo de petición:",
      exampleName: "Nombre: Carlos Vicioso",
      exampleEmail: "Email: tu.email@ejemplo.com",
      exampleType: "Tipo: Oferta de Trabajo",
      exampleMessage:
        "Mensaje: Hola, me gustaría ofrecerte una posición como desarrollador backend en nuestra empresa. El proyecto involucra tecnologías modernas y un equipo dinámico. ¿Te interesaría conocer más detalles?",
      exampleNote:
        "Al enviar, recibirás una confirmación y te contactaré pronto para discutir los detalles.",
      monday: "Lunes",
      tuesday: "Martes",
      wednesday: "Miércoles",
      thursday: "Jueves",
      friday: "Viernes",
      mon: "Lun",
      tue: "Mar",
      wed: "Mié",
      thu: "Jue",
      fri: "Vie",
      success: "Petición enviada exitosamente",
      error: "Error al enviar la petición",
    },
    // NotFound
    notFound: {
      gameOver: "Game Over",
      fileNotFound: "404 FILE NOT FOUND",
      backToHome: "Back to Home",
    },
    // Admin
    admin: {
      title: "Panel de Administración",
      restrictedAccess: "Acceso Restringido",
      login: "Iniciar Sesión",
      username: "Usuario",
      password: "Contraseña",
      logout: "Cerrar Sesión",
      invalidCredentials: "Usuario o contraseña incorrectos",
      contentManagement: "Gestión de Contenido",
      groups: {
        dashboard: "Panel",
        content: "Contenido",
        general: "General",
        system: "Sistema",
      },
      overview: "Resumen General",
      viewSite: "Ver el sitio",
      externalTools: "Herramientas Externas",
      tools: {
        ga4: "Google Analytics 4",
        ga4Desc: "Analítica de tráfico del portfolio; alimenta el BOS con su Data API gratuita.",
        console: "Google Search Console",
        consoleDesc: "Rendimiento e índice del sitio en Google.",
        bing: "Bing Webmasters",
        bingDesc: "SEO y rendimiento de vixis.dev en Bing.",
        betterstack: "BetterStack",
        betterstackDesc: "Uptime y logs de los servicios en producción.",
      },
      blogPosts: "Blog Posts",
      homeContent: "Contenido del Inicio",
      radioSettings: "Configuración de Radio",
      invoices: "Facturas",
      appearance: "Apariencia",
      appearanceContractsBg: "Fondo de Contratos",
      appearanceLegalBg: "Fondo de páginas legales (Privacidad y Términos)",
      appearanceDefault: "Por defecto",
      appearanceStarry: "Starry Night (Canvas Animado)",
      appearanceBrand: "Marca (emails y documentos)",
      brandName: "Nombre de marca",
      brandLogo: "Logo (URL)",
      brandColor: "Color principal (botones)",
      brandFooter: "Pie de email",
      jobOffers: "Ofertas de Trabajo",
      crm: {
        label: "CRM",
        subtitle: "Contactos, empresas, pipeline de ventas, actividades, leads, emails y contratos",
        loading: "Cargando CRM…",
        error: "Error:",
        tabs: {
          contacts: "Contactos",
          companies: "Empresas",
          pipeline: "Pipeline",
          activities: "Actividades",
          leads: "Leads y visitas",
          emails: "Emails",
          contracts: "Contratos",
          integrations: "Integraciones",
        },
        newCompany: "+ Empresa",
        newContact: "+ Contacto",
        newDeal: "+ Deal",
        newContract: "+ Contrato",
        save: "Guardar",
        saving: "Guardando…",
        cancel: "Cancelar",
        edit: "Editar",
        delete: "Eliminar",
        send: "Enviar",
        sending: "Enviando…",
        close: "Cerrar",
        exportCsv: "Exportar CSV",
        importCsv: "Importar CSV",
        importing: "Importando…",
        noDataExport: "No hay datos para exportar",
        emptyCsv: "CSV vacío o sin cabecera",
        missingColumn: "Falta la columna",
        contact: "Contacto",
        contacts: "contactos",
        company: "Empresa",
        companies: "empresas",
        imported: "Importados",
        importedF: "Importadas",
        createdCompanies: "empresas creadas",
        rowsSkipped: "filas sin nombre omitidas",
        linkCopied: "Enlace copiado",
        sentOk: "Email enviado correctamente",
        errSaveContact: "Error al guardar contacto",
        errSaveCompany: "Error al guardar empresa",
        errSaveDeal: "Error al guardar deal",
        errMoveDeal: "Error al mover el deal",
        errSaveActivity: "Error al guardar actividad",
        errUpdateActivity: "Error al actualizar actividad",
        errConvertLead: "Error al convertir el lead",
        errSaveTemplate: "Error al guardar plantilla",
        errDelete: "Error al eliminar",
        errSendEmail: "Error al enviar email",
        errSaveContract: "Error al guardar contrato",
        errSign: "Error al firmar",
        errRegen: "Error al regenerar",
        errImport: "Error al importar CSV",
        mapTitle: "Revisar columnas",
        mapHint:
          "Detectamos las columnas de tu archivo automáticamente. Ajusta lo que haga falta; los campos sin columna se dejan vacíos.",
        mapIgnore: "Ignorar",
        mapSourceCol: "Columna del archivo",
        mapImport: "Importar",
        mapUnmatched: "Columnas no usadas",
        mapDetected: "detectadas",
        duplicatesSkipped: "duplicados omitidos",
        vcardDetected: "Archivo vCard detectado",
        noUsableColumns:
          "No pudimos reconocer ninguna columna. Revisa el archivo o ajusta el mapeo manualmente.",
        fieldFirstName: "Nombre",
        fieldLastName: "Apellido",
        fieldFullName: "Nombre completo",
        fieldEmail: "Email",
        fieldPhone: "Teléfono",
        fieldPhone2: "Teléfono 2",
        fieldCompany: "Empresa",
        fieldJobTitle: "Cargo",
        fieldLinkedin: "LinkedIn",
        fieldGithub: "GitHub",
        fieldXHandle: "X / Twitter",
        fieldWebsite: "Sitio web",
        fieldAddress: "Dirección",
        fieldBirthdate: "Cumpleaños",
        fieldGender: "Género",
        fieldExperience: "Años de experiencia",
        fieldOwner: "Responsable",
        fieldTags: "Etiquetas",
        fieldSource: "Origen",
        fieldNotes: "Notas",
        fieldDomain: "Dominio",
        fieldIndustry: "Industria",
        fieldName: "Nombre",
        fieldCompanyName: "Nombre",
        confirmDeleteTemplate: "¿Eliminar plantilla? Los envíos previos se conservan.",
        confirmDeleteContract: "¿Eliminar contrato? El enlace público dejará de funcionar de inmediato. Esta acción es irreversible.",
        confirmDeleteContact: "¿Eliminar contacto? Sus actividades se borrarán y sus deals quedarán sin persona.",
        confirmDeleteCompany: "¿Eliminar empresa? Sus contactos y deals quedarán sin empresa.",
        confirmDeleteDeal: "¿Eliminar deal? Sus actividades se borrarán.",
        confirmDeleteActivity: "¿Eliminar actividad?",
        confirmRegenPwd: "Regenerar contraseña del contrato. La anterior dejará de funcionar. ¿Continuar?",
        signPrompt: "Nombre del firmante (proveedor):",
        contactsTab: {
          searchPh: "Buscar por nombre, email, teléfono o etiqueta…",
          thContact: "Contacto",
          thCompany: "Empresa",
          thEmailPhone: "Email / Teléfono",
          thTags: "Etiquetas",
          thOrigin: "Origen",
          noEmail: "Sin email",
          noPhone: "Sin teléfono",
          sortBy: "Ordenar",
          sortName: "Nombre A–Z",
          sortCompany: "Empresa",
          sortRecent: "Recientes",
          sortOldest: "Antiguos",
          filterSource: "Origen",
          allSources: "Todos",
        },
        tabContact: "Contacto",
        tabCompany: "Empresa",
        grid: {
          search: "Buscar {{entity}}…",
          fields: "Campos",
          new: "+ {{entity}}",
          open: "Abrir",
          deleteRow: "Eliminar",
          clickToEdit: "clic para editar",
          addField: "Añadir campo",
          sortAsc: "Ordenar ▲",
          sortDesc: "Ordenar ▼",
          hideColumn: "Ocultar columna",
          save: "Guardar",
          cancel: "Cancelar",
          required: "{{label}} (obligatorio)",
          creatingLabel: "Creando {{entity}}…",
          empty: "No hay {{entity}}s todavía.",
          noMatches: "Sin resultados para la búsqueda.",
          count: "{{n}} registros",
          deleteSelected: "Eliminar ({{n}})",
          deleteSelectedConfirm: "¿Eliminar",
          bulkEdit: "Editar ({{n}})",
          bulkTitle: "Edición masiva ({{n}})",
          bulkField: "Campo",
          bulkApply: "Aplicar a {{n}}",
          bulkAdd: "Añadir",
          bulkRemove: "Quitar",
          clearSelection: "Limpiar selección",
          selectAll: "Seleccionar todo",
          selectRow: "Seleccionar fila",
          rowMenu: "Opciones",
          saveError: "No se pudo guardar. Los cambios se revirtieron.",
          createError: "No se pudo crear el registro.",
        },
        drawer: {
          title: "Detalle",
          relationship: "Relaciones",
          delete: "Eliminar",
          close: "Cerrar",
        },
        fields: {
          title: "Campos de {{entity}}",
          add: "Añadir campo",
          close: "Cerrar",
          labelField: "Nombre del campo",
          labelPh: "Ej: Redes sociales",
          optionsPh: "Opciones separadas por coma (solo select/multi)",
          nameRequired: "Escribe un nombre para el campo.",
          nameTaken: "Ya existe un campo con ese nombre.",
          saveField: "Guardar campo",
          toggle: "Mostrar / ocultar",
          system: "sistema",
          readonly: "solo lectura",
          hide: "Ocultar",
          show: "Mostrar",
          showAll: "Mostrar todos",
          hideAll: "Ocultar todos",
          visibleOf: "{{v}} de {{n}} visibles",
          delete: "Eliminar",
          confirmDelete: "¿Eliminar este campo? Los datos guardados se borrarán.",
          error: "No se pudo guardar el campo.",
        },
        contactModal: {
          new: "Nuevo contacto",
          edit: "Editar contacto",
          firstName: "Nombre *",
          firstNamePh: "Nombre",
          lastName: "Apellido",
          lastNamePh: "Apellido",
          email: "Email",
          emailPh: "correo@dominio.com",
          phone: "Teléfono",
          phonePh: "+57 …",
          company: "Empresa",
          noCompany: "Sin empresa",
          origin: "Origen",
          srcManual: "Manual",
          srcTally: "Tally",
          srcWhatsapp: "WhatsApp",
          srcPortfolio: "Portfolio",
          jobTitle: "Puesto",
          jobTitlePh: "Ej: CTO, Diseñadora…",
          phone2: "Teléfono 2",
          birthdate: "Nacimiento",
          gender: "Género",
          genderUnspecified: "Sin especificar",
          genderFemale: "Femenino",
          genderMale: "Masculino",
          genderOther: "Otro",
          linkedin: "LinkedIn",
          github: "GitHub",
          xHandle: "X (Twitter)",
          website: "Web",
          address: "Dirección",
          experienceYears: "Años exp.",
          owner: "Propietario",
          tags: "Etiquetas (separadas por coma)",
          tagsPh: "lead, client…",
          notes: "Notas",
          notesPh: "Contexto, preferencias…",
        },
        contactDetail: {
          noEmail: "Sin email",
          noPhone: "Sin teléfono",
          deals: "Deals",
          noDeals: "Sin deals asociados.",
          activities: "Actividades",
          noActivities: "Sin actividades.",
          addActivity: "+ Actividad",
          sendEmail: "Enviar email",
          memberSince: "En CRM desde",
          profile: "Perfil",
          social: "Redes y web",
          extra: "Datos adicionales",
        },
        companiesTab: {
          count: "empresas",
          thCompany: "Empresa",
          thIndustry: "Industria",
          thContacts: "Contactos",
          thDeals: "Deals",
          thDomain: "Dominio",
          empty: "No hay empresas. Crea una con \"+ Empresa\" (el logo se obtiene del dominio automáticamente).",
          sortBy: "Ordenar",
          sortName: "Nombre A–Z",
          sortContacts: "Más contactos",
        },
        companyModal: {
          new: "Nueva empresa",
          edit: "Editar empresa",
          name: "Nombre *",
          namePh: "Nombre de la empresa",
          domain: "Dominio (obtiene el logo)",
          domainPh: "dominio.com",
          industry: "Industria",
          industryPh: "Ej: SaaS, Desarrollo…",
          notes: "Notas",
          notesPh: "Detalles/contexto",
          logoNote: "Logo actual (se actualiza automáticamente al cambiar el dominio)",
          tags: "Etiquetas (separadas por coma)",
          tagsPh: "client, partner…",
          foundedYear: "Año fundado",
          employeeRange: "Rango empleados",
          empRange1: "1–10",
          empRange2: "11–50",
          empRange3: "51–200",
          empRange4: "201–1000",
          empRange5: "1000+",
          nit: "NIT / ID fiscal",
          nitPh: "900123456-7",
          address: "Dirección",
          phone: "Teléfono",
          linkedin: "LinkedIn",
          github: "GitHub",
          xHandle: "X (Twitter)",
          website: "Web",
          owner: "Propietario",
        },
        pipeline: {
          newHere: "Nuevo deal en esta etapa",
          moveTo: "Mover a",
          close: "Cierre:",
          noDeals: "Sin deals",
          noStages: "No hay etapas configuradas.",
        },
        dealModal: {
          new: "Nuevo deal",
          edit: "Editar deal",
          title: "Título *",
          titlePh: "Ej: Web corporativa",
          contact: "Contacto",
          noContact: "Sin contacto",
          company: "Empresa",
          noCompany: "Sin empresa",
          stage: "Etapa",
          value: "Valor",
          currency: "Moneda",
          prob: "Prob. %",
          closeDate: "Cierre esperado",
        },
        activitiesTab: {
          allPeople: "Todas las personas",
          addActivity: "+ Actividad",
          thType: "Tipo",
          thSubject: "Asunto",
          thPerson: "Persona",
          thDue: "Fecha límite",
          thDone: "Hecho",
          empty: "No hay actividades. Crea una con \"+ Actividad\".",
        },
        activityModal: {
          new: "Nueva actividad",
          edit: "Editar actividad",
          type: "Tipo",
          typeNote: "Nota",
          typeEmail: "Email",
          typeCall: "Llamada",
          typeTask: "Tarea",
          typeMeeting: "Reunión",
          due: "Fecha límite",
          subject: "Asunto *",
          subjectPh: "Asunto",
          person: "Persona",
          noPerson: "Sin persona",
          deal: "Deal",
          noDeal: "Sin deal",
          detail: "Detalle",
          detailPh: "Notas / seguimiento",
        },
        leadsTab: {
          title: "Leads",
          converted: "convertidos",
          pending: "por tratar",
          noName: "Lead sin datos",
          toContact: "+ Contacto",
          converting: "Convirtiendo…",
          inCrm: "En CRM",
          noLeads: "Sin leads todavía",
          visitorsTitle: "Visitantes del portfolio",
          today: "hoy",
          total: "total",
          noVisits: "Sin visitas registradas",
          dedupeNote: "Una visita por sesión + página + día (dedupe automático).",
          from: "desde",
          country: "País",
          device: "Dispositivo",
          page: "Página",
          campaign: "Campaña",
        },
        contractTypes: {
          servicios: "Prestación de Servicios",
          consultoria: "Consultoría",
          nda: "Confidencialidad (NDA)",
          oferta: "Oferta Comercial",
          soporte: "Soporte y Mantenimiento",
          licencia: "Licenciamiento",
          otro: "Otro",
        },
        emailsTab: {
          hint: "Plantillas con placeholders —",
          newTemplate: "+ Plantilla",
          templates: "Plantillas",
          active: "activa",
          paused: "pausada",
          noTemplates: "Sin plantillas. Crea una para poder enviar emails.",
          recent: "Envíos recientes",
          noSends: "Sin envíos todavía.",
          noSubject: "sin asunto",
          sent: "Enviado",
          pending: "Pendiente",
          failed: "Error",
        },
        templateModal: {
          new: "Nueva plantilla",
          edit: "Editar plantilla",
          name: "Nombre *",
          namePh: "Oferta inicial",
          status: "Estado",
          subject: "Asunto *",
          subjectEn: "Asunto en inglés (opcional)",
          body: "Cuerpo (texto plano, con placeholders)",
          bodyEn: "Cuerpo en inglés (opcional)",
          placeholdersNote: "Placeholders disponibles:",
        },
        sendModal: {
          title: "Enviar email",
          contact: "Contacto *",
          select: "Selecciona…",
          noEmailOpt: "sin email",
          template: "Plantilla *",
          pausedOpt: "[pausada] ",
          deal: "Deal (opcional)",
          noDeal: "Sin deal",
          lang: "Idioma",
          preview: "Vista previa (con datos del contacto):",
          noEmailWarn: "El contacto seleccionado no tiene email. Añádelo antes de enviar.",
        },
        contractsTab: {
          desc: "Contratos con link público protegido por contraseña. Al firmarlo ambas partes queda descargable en PDF.",
          pwdLabel: "Contraseña de acceso:",
          copiedClose: "Copiada — cerrar",
          thContract: "Contrato",
          thClient: "Cliente",
          thAmount: "Importe",
          thStatus: "Estado",
          thLink: "Enlace",
          thActions: "Acciones",
          created: "Creado",
          signed: "Firmado ✓",
          partial: "Firma parcial",
          sent: "Enviado",
          copyLink: "Copiar link",
          sign: "Firmar",
          signedBtn: "Firmado",
          key: "Clave",
          noContracts: "Sin contratos.",
          failedTip: "intentos fallidos (24h)",
        },
        integrationsTab: {
          desc: "Conecta Google Contacts o importa archivos vCard/CSV. La deduplicación evita crear contactos repetidos.",
          notConnected: "Sin conectar",
          connect: "Conectar con Google",
          sync: "Sincronizar contactos",
          working: "Procesando…",
          disconnect: "Desconectar",
          status: "Estado",
          lastSync: "Última sync",
          imported: "Nuevos",
          linked: "vinculados",
          skipped: "omitidos",
          syncDone: "Sincronización completada",
          confirmDisconnect: "¿Desconectar Google Contacts del CRM?",
          notConfigured: "Faltan GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET en los secretos de Supabase.",
          err: "Error en la integración",
          fileTitle: "Importar archivos",
          fileDesc: "vCard (.vcf) de Google/Outlook o CSV con cualquier orden de columnas.",
          importContacts: "Importar contactos",
          importCompanies: "Importar empresas",
          fileFormats: "Formatos: .vcf, .vcard, .csv",
        },
        contractModal: {
          new: "Nuevo contrato",
          edit: "Editar contrato",
          title: "Título *",
          titlePh: "Proyecto web — alcance inicial",
          titleEn: "Título en inglés (opcional)",
          ctype: "Tipo de contrato",
          contact: "Contacto",
          noContact: "Sin contacto",
          company: "Empresa",
          noCompany: "Sin empresa",
          currency: "Moneda",
          amount: "Importe",
          terms: "Condiciones *",
          termsPh: "1. Alcance del proyecto\n2. Plazos\n3. Pagos\n…",
          termsEn: "Condiciones en inglés (opcional)",
          editNote: "Editar no cambia la contraseña actual. La firma se gestiona desde la tabla.",
          createNote: "Al crear el contrato se genera la contraseña y el enlace público automáticamente.",
        },
      },
      bos: {
        subtitle: "Rendimiento integral del negocio",
        revenue30: "Ingresos 30 días",
        revenue90: "Ingresos 90 días",
        lifetime: "De por vida",
        ticket: "Ticket promedio 30d",
        orders: "pedidos",
        revenueChart: "Ingresos — últimos 30 días",
        trafficChart: "Tráfico web — 30 días",
        recentSales: "Últimas ventas",
        noSales: "Aún no hay ventas pagadas",
        byGateway: "Ingresos por pasarela (90d)",
        noData: "Sin datos",
        leads: "Leads",
        total: "en total",
        noLeads: "Sin leads todavía",
        converted: "convertidos",
        pending: "por tratar",
        toCrm: "→ CRM",
        inCrm: "En CRM",
        noName: "Lead sin datos",
        visitors: "Visitantes del portfolio",
        today: "hoy",
        totalVisitors: "total registrados",
        noVisits: "Aún sin visitas registradas",
        connectors: "Conectores",
      },
      mediaManagement: "Gestión de Recursos Multimedia",
      show: "Mostrar",
      hide: "Ocultar",
      mediaResources: "Recursos multimedia",
      changeStatus: "Cambiar Estado",
      products: "Productos",
      projects: "Proyectos",
      clients: "Abonadores",
      testimonials: "Testimonios",
      socials: "Redes Sociales",
      events: "Eventos",
      workExperiences: "Experiencia Laboral",
      technologies: "Tecnologías",
      studies: "Estudios",
      noItems: "No hay elementos disponibles",
      createNew: "Crear nuevo",
      editItem: "Editar elemento",
      deleteItem: "Eliminar elemento",
      saveSuccess: "Elemento guardado exitosamente",
      deleteSuccess: "Elemento eliminado exitosamente",
      error: "Error al procesar la solicitud",
      noProducts: "No hay productos disponibles",
      noProjects: "No hay proyectos disponibles",
      noClients: "No hay abonadores disponibles",
      noTestimonials: "No hay testimonios disponibles",
      noSocials: "No hay redes sociales disponibles",
      noEvents: "No hay eventos disponibles",
      noWorkExperiences: "No hay experiencias laborales disponibles",
      noTechnologies: "No hay tecnologías disponibles",
      noStudies: "No hay estudios disponibles",
      noInvoices: "No hay facturas disponibles",
      createNewToStart: "Crea uno nuevo para comenzar.",
      radioControl: "Control de Radio",
      icecastStatus: "Estado de Icecast",
      liquidsoapStatus: "Estado de Liquidsoap",
      cloudflareTunnelStatus: "Estado de Cloudflare Tunnel",
      radioFrontend: "Frontend de Radio",
      cloudflareDashboard: "Dashboard de Cloudflare",
      active: "Activo",
      inactive: "Inactivo",
      unknown: "Desconocido",
      // Títulos de modales
      editProduct: "Editar Producto",
      createProduct: "Crear Producto",
      editProject: "Editar Proyecto",
      createProject: "Crear Proyecto",
      editClient: "Editar Abonador",
      createClient: "Crear Abonador",
      editTestimonial: "Editar Testimonio",
      createTestimonial: "Crear Testimonio",
      editSocial: "Editar Red Social",
      createSocial: "Crear Red Social",
      editEvent: "Editar Evento",
      createEvent: "Crear Evento",
      editWorkExperience: "Editar Experiencia Laboral",
      createWorkExperience: "Crear Experiencia Laboral",
      editTechnology: "Editar Tecnología",
      createTechnology: "Crear Tecnología",
      editStudy: "Editar Estudio",
      createStudy: "Crear Estudio",
      // Campos de formularios
      fieldTitle: "Título",
      fieldDescription: "Descripción",
      fieldFullDescription: "Descripción Completa",
      fieldBasePrice: "Precio Base (COP)",
      fieldThumbnailUrl: "Thumbnail URL",
      fieldActionType: "Tipo de Acción",
      fieldActionUrl: "Action URL",
      fieldPricingLink: "Pricing Link",
      fieldButtonText: "Texto del Botón",
      fieldUrl: "URL",
      fieldRepository: "Repositorio",
      fieldMonth: "Mes",
      fieldYear: "Año",
      fieldName: "Nombre",
      fieldLogo: "Logo URL",
      fieldTestimonialContent: "Contenido del Testimonio",
      fieldTestimonialAuthor: "Autor del Testimonio",
      fieldTestimonialRole: "Rol del Autor",
      fieldTestimonialUrl: "URL del Testimonio",
      fieldIcon: "Icono URL",
      fieldPosition: "Posición",
      fieldCompany: "Empresa",
      fieldCompanyUrl: "URL Empresa",
      fieldCompanyLogo: "Logo Empresa URL",
      fieldLocation: "Ubicación",
      fieldStartDate: "Fecha Inicio",
      fieldEndDate: "Fecha Fin",
      leaveEmptyIfCurrent: "Dejar vacío si es trabajo actual",
      fieldResponsibilities: "Responsabilidades (JSON Array)",
      fieldTechnologies: "Tecnologías (JSON Array)",
      fieldType: "Tipo",
      fieldTypeOptionCourse: "Curso",
      fieldTypeOptionCertification: "Certificación",
      fieldTypeOptionDegree: "Título Universitario",
      fieldStatus: "Estado",
      completed: "Completado",
      inProgress: "En curso",
      fieldInstitution: "Institución",
      fieldCertificateUrl: "URL Certificado",
      fieldYearsOfExperience: "Años de Experiencia",
      fieldCategory: "Categoría",
      fieldLevel: "Nivel",
      // Opciones de formularios
      actionTypeLink: "Link",
      actionTypeSubmit: "Submit",
      actionTypeSchedule: "Schedule",
      // Media Manager
      resourceType: "Tipo de Recurso",
      selectFile: "Seleccionar Archivo",
      optimizeAndUpload: "Optimizar y Subir",
      uploading: "Subiendo...",
      uploadSuccess:
        "¡Archivo subido exitosamente! URL copiada al portapapeles.",
      uploadError: "Error al subir archivo",
      urlCopied: "URL copiada al portapapeles",
      copyUrl: "Copiar URL",
      // Opciones de recursos
      productThumbnails: "Thumbnails de Productos",
      productImages: "Imágenes de Productos",
      eventThumbnails: "Thumbnails de Eventos",
      generalAssets: "Assets Generales",
      // Mensajes
      confirmDelete: "¿Estás seguro de que quieres eliminar este elemento?",
      deleteWarning: "Esta acción no se puede deshacer.",
      errorDeleting: "Error al eliminar",
      errorSaving: "Error al guardar",
      errorUnknown: "Error desconocido",
      errorGeneratingLink: "Error al generar pricing link",
      errorExtractingData: "Error al extraer datos",
      pleaseEnterUrl: "Por favor ingresa una URL válida",
      generateLink: "Generar",
      extractEventData: "Extraer Datos del Evento",
      eventUrl: "URL del Evento",
      eventDate: "Fecha del Evento",
      supportsEventPlatforms:
        "Soporta Passline, start.gg, duel.plus y otras plataformas",
      clientCannotChange: "El abonador no se puede cambiar al editar",
      buttonTextPlaceholder: "Ej: Comprar, Agéndalo, etc.",
      projectStatus: "Estado del Proyecto",
      noDate: "Sin fecha",
      noDescription: "Sin descripción",
      noTestimonial: "Sin testimonio",
      noClient: "Abonador",
    },
    // Status Badge
    statusBadge: {
      confirm:
        "¿Estás seguro de que quieres continuar? Puede que yo no te responda debido a diversas razones...",
      available: "Disponible para nuevos proyectos",
      away: "Ausente",
      busy: "No disponible",
    },
    // Contact Section
    contactSection: {
      title: "Contactar",
      description:
        "Creando experiencias digitales únicas. ¡Hablemos sobre tu próximo proyecto!",
      sendRequest: "Envíame tu propuesta",
      contact: "Contáctame en redes",
    },
    // Home Section
    homeSection: {
      projects: "Proyectos",
    },
    // Latest Post Card
    latestPostCard: {
      title: "Último Post",
      readMore: "Leer más",
    },
    // Legal
    legal: {
      privacyTitle: "Política de Privacidad",
      termsTitle: "Términos y Condiciones",
      privacyLink: "Política de Privacidad",
      termsLink: "Términos y Condiciones",
    },
    // How to Pay Me
    howToPay: {
      title: "¿Cómo pagarme?",
      description: "Sigue estos simples pasos para completar tu pago:",
      payButton: "Pagar ahora",
      step1: "Asegúrate de incluir el ID del producto y el número de factura en la nota de pago.",
      step2: "Tu pago será procesado dentro de 24-48 horas después de la confirmación.",
      step3: "Si tienes alguna pregunta, por favor contáctame a través de mis redes sociales.",
      poweredBy: "Pagos seguros impulsados por",
      paymentMethods: "+500 métodos de pago",
      paymentDetails: "Transferencias bancarias, cripto, pagos móviles y más. Cancela en cualquier momento.",
    },
    // Checkout Propio
    checkout: {
      title: "Finalizar compra",
      backToProduct: "Volver al producto",
      backToStore: "Volver a la tienda",
      productNotFound: "Producto no encontrado",
      invoiceNotFound: "Factura no encontrada",
      invoiceNotFoundHint: "El enlace es inválido o la factura ya no existe.",
      goHome: "Volver al inicio",
      error: "Error",
      buyerInfo: "Información del comprador",
      fullName: "Nombre completo",
      email: "Email",
      deliveryNote:
        "Recibirás tu factura y el acceso al producto en este email.",
      nameRequired: "Tu nombre es requerido",
      emailRequired: "Tu email es requerido",
      emailInvalid: "Email inválido",
      tabCard: "Tarjeta",
      tabCrypto: "Criptomonedas",
      payWithCard: "Pagar con tarjeta (Visa, Mastercard, Amex, PSE)",
      cardInfo:
        "Pago seguro procesado por dLocal. Tarjetas de crédito, débito y métodos locales.",
      payCardButton: "Pagar con tarjeta",
      confirmCardButton: "Confirmar pago",
      documentType: "Tipo de documento",
      documentNumber: "Número de documento",
      documentRequired: "El número de documento es requerido",
      country: "País",
      countryRequired: "Selecciona tu país",
      selectCountry: "Selecciona tu país",
      cardTokenError: "No se pudo procesar la tarjeta, verifica los datos",
      cardUnavailable:
        "El pago con tarjeta no está disponible en este momento.",
      secureNote:
        "Los datos de tu tarjeta se procesan de forma segura por dLocal (PCI DSS).",
      payWithCrypto: "Pagar con criptomonedas",
      cryptoInfo:
        "Acepto BTC, ETH, USDT y más de 300 criptomonedas. Recibirás un enlace de pago seguro.",
      payCryptoButton: "Pagar con criptomonedas",
      paypalUnavailable: "PayPal no está disponible en este momento.",
      paypalError: "Ocurrió un error con PayPal. Inténtalo de nuevo.",
      creatingPayment: "Creando pago seguro...",
      waitingConfirmation: "Esperando confirmación del pago...",
      confirmingPayment: "Confirmando tu pago...",
      stillWaiting:
        "Tu pago está siendo procesado. Te enviaremos el acceso por email.",
      paymentFailed: "El pago no se pudo completar.",
      noRedirect: "No se pudo obtener el link de pago",
      successTitle: "¡Pago confirmado!",
      successMessage:
        "¡Pago confirmado! Te he enviado la factura y el acceso a tu producto por email.",
      invoiceLabel: "Factura",
      yourAccess: "Accede a tu producto:",
      done: "Listo",
    },
  },
  en: {
    // Navigation
    nav: {
      home: "Home",
      studio: "Studio",
      about: "About",
      work: "Work",
      projects: "Projects",
      clients: "Payers",
      workExperience: "Experience",
      stack: "Stack",
      studies: "Studies",
      socials: "Social Media",
      store: "Store",
      radio: "Radio",
      blog: "Blog",
      settings: "Settings",
      language: "Language",
      donations: "Donations",
      code: "Code",
      repository: "Repository",
    },
    // Common pages
    common: {
      loading: "Loading...",
      error: "Error",
      noContent: "No content available",
      back: "Back",
      close: "Close",
      save: "Save",
      cancel: "Cancel",
      delete: "Delete",
      edit: "Edit",
      create: "Create",
      update: "Update",
      next: "Next",
      previous: "Previous",
      page: "Page",
      of: "of",
      selectOption: "Select an option",
      optional: "Optional",
    },
    // Home / Hero
    home: {
      title: "Carlos Andres Vicioso Lara",
      subtitle: "Systems Engineer, Colombia.",
      specialization: "Specialized in backend development.",
    },
    // Timeline
    timeline: {
      checkpoint1: {
        year: "2015",
        title: "SQL at 13 years old",
        description: "I downloaded XAMPP, explored PhpMyAdmin, and discovered my first machine language by running SQL code.",
      },
      checkpoint2: {
        year: "2017",
        title: "HTML + Logic Training",
        description: "I learned HTML in school, and I learned its formats on the internet. I also trained my programming logic by playing video games.",
      },
      checkpoint3: {
        year: "2019",
        title: "Software Analysis and Development",
        description: "Flowcharts, pseudocode, CSS, Java, JavaScript, Python, PHP, Laravel, architecture, and more.",
      },
      checkpoint4: {
        year: "2020",
        title: "Getting Started with PHP",
        description: "Despite the pandemic, I learned PHP and Laravel at the same institute as last year.",
      },
      checkpoint5: {
        year: "2022",
        title: "Exploring more of the career with the University",
        description: "I managed to advance by training with cybersecurity, cloud, AI, and more technologies.",
      },
      checkpoint6: {
        year: "2024-2025",
        title: "Automation & AI + Degree",
        description: "Specialization in artificial intelligence and automation + Diploma in Cybersecurity + Degree in Systems Engineering",
      },
      checkpoint7: {
        year: "2026",
        title: "Vixis Studio",
        description: "Established business brand. Development, automation, and more services.",
      },
      close: "Close",
    },
    // About
    about: {
      title: "About Me",
      description1:
        "I am systems engineer passionate about technology and music, better known as Vixis. I specialize in backend development, process automation, and artificial intelligence.",
      description2:
        "My goal is not only to create secure, efficient, and scalable solutions, but also to add value to the community and achieve projects for my payers. I help personal brands, entrepreneurs, and remote teams save time, reduce errors, and scale operations through automated or programmed flows with different tools or technologies.",
      description3:
        "I really enjoy frontend development and video games, cybersecurity, and automation; but I also love server-side development and mobile app development.",
      followSocials: "Follow me on social media",
    },
    // Store
    store: {
      title: "Store",
      noProducts: "No products available",
      noProductsDescription: "Come back soon to see my products",
      noProductsInCategory: "No products in this category",
      tryAnotherCategory: "Try selecting another category",
      buy: "Buy",
      schedule: "Schedule",
      request: "Request",
      viewPricing: "View Pricing",
      price: "Price",
      free: "Free",
      searchPlaceholder: "Search by name...",
      allPrices: "All",
      description: "Description",
      fullDescription: "Full Description",
      scheduleSoon: "Scheduling functionality coming soon",
      sendingRequest: "Sending request...",
      filterByCategory: "Filter by category:",
      allCategories: "All categories",
      showingResults: "Showing",
      of: "of",
      products: "products",
      categoryLabel: "Category",
      category: {
        programming: "Programming",
        clothing: "Clothing",
        design: "Design",
        consulting: "Consulting",
        course: "Course",
        investment: "Investment",
        music: "Music",
        languages: "Languages",
      },
      paymentInfo: {
        title: "Payment Information",
        popoverTitle: "Payment Instructions",
        popoverDescription: "Click below for detailed payment instructions and video tutorial.",
        viewGuide: "View Payment Guide →",
      },
    },
    // Projects
    projects: {
      title: "Projects",
      loading: "Loading projects...",
      noProjects: "No projects available",
      noProjectsDescription: "Come back soon to see my projects",
      visit: "Visit",
      repository: "Repository",
      special: "Special",
    },
    // Clients
    clients: {
      title: "Payers",
      loading: "Loading payers...",
      noClients: "No payers available",
      noClientsDescription: "Come back soon to see my payers",
      testimonials: "Testimonials",
      noTestimonials: "No testimonials available",
      visit: "Visit",
      viewTestimonial: "View testimonial",
      viewLink: "View link",
      errorLoadingClients: "Error loading payers:",
    },
    // Work Experience
    workExperience: {
      title: "Experience",
      loading: "Loading experiences...",
      noExperiences: "No experiences available",
      noExperiencesDescription:
        "Come back soon to see my professional experience",
      noResults: "No experiences of this type",
      downloadCV: "Download CV",
      position: "Position",
      company: "Company",
      location: "Location",
      period: "Period",
      description: "Description",
      responsibilities: "Responsibilities",
      achievements: "Achievements",
      technologies: "Technologies",
      evidence: "Evidence",
      evidenceImage: "Photo",
      evidenceVideo: "Video",
      evidenceDocument: "Document",
      evidenceLink: "Link",
      viewEvidence: "View evidence",
      category: "Type",
      allCategories: "All",
      categoryLaboral: "Work",
      categoryProfesional: "Professional",
      categoryRelacionada: "Related",
      categoryFreelance: "Freelance",
      categoryPracticas: "Internship",
      categoryDocencia: "Teaching",
      categoryInvestigacion: "Research",
      categoryVoluntariado: "Volunteering",
      type: "Type",
      status: "Status",
      current: "Current",
      past: "Past",
      fullTime: "Full Time",
      partTime: "Part Time",
      contract: "Contract",
      freelance: "Freelance",
      present: "Present",
      assistant: "Systems Technical Assistant",
      discordMarketer: "Discord Marketer",
      fullStackDeveloper: "Full-Stack Developer",
      january: "January",
      february: "February",
      march: "March",
      may: "May",
      april: "April",
      june: "June",
      july: "July",
      august: "August",
      september: "September",
      october: "October",
      november: "November",
      december: "December",
    },
    // Studies
    studies: {
      title: "Studies",
      loading: "Loading studies...",
      noStudies: "No studies available",
      noStudiesDescription: "Come back soon to see my studies",
      filter: "Filter",
      clearFilters: "Clear filters",
      search: "Search",
      titlePlaceholder: "Search by title...",
      institutionPlaceholder: "Search by institution...",
      type: "Type",
      status: "Status",
      all: "All",
      degree: "University Degree",
      certification: "Certification",
      course: "Course",
      completed: "Completed",
      inProgress: "In Progress",
      hasCertificate: "Has Certificate",
      yes: "Yes",
      no: "No",
      viewCertificate: "View Certificate",
      institution: "Institution",
      period: "Period",
      sortByPeriod: "Sort by period",
      description: "Description",
      present: "Present",
    },
    // Skills & Technologies
    skills: {
      title: "Tech Stack",
      loading: "Loading technologies...",
      noTechnologies: "No technologies available",
      noTechnologiesDescription: "Come back soon to see my technologies",
      filter: "Filter",
      clearFilters: "Clear filters",
      search: "Search",
      namePlaceholder: "Search by name...",
      category: "Category",
      level: "Level",
      experience: "Experience",
      all: "All",
      language: "Language",
      framework: "Framework",
      database: "Database",
      tool: "Tool",
      cloud: "Cloud",
      instrument: "Instrument",
      music: "Music",
      other: "Other",
      beginner: "Beginner",
      intermediate: "Intermediate",
      advanced: "Advanced",
      expert: "Expert",
      years: "years",
      ofExperience: "of experience",
    },
    // Socials
    socials: {
      title: "Social Media",
      loading: "Loading...",
      noSocials: "No social media available",
      noSocialsDescription: "Come back soon to see my social media",
      visit: "Visit",
      description: "Description",
      errorLoading: "Error loading social media:",
    },
    // Radio
    radio: {
      title: "Radio Vixis",
      viewMore: "View more",
      live_home: "Live",
      live: "LIVE",
      online: "ONLINE",
      offline: "OFFLINE",
      playing: "Playing",
      paused: "Paused",
      volume: "Volume",
      mute: "Mute",
      unmute: "Unmute",
      play: "Play",
      pause: "Pause",
      refresh: "Refresh",
      previous: "Previous",
      next: "Next",
      loading: "Loading...",
      offlineTitle: "Radio offline",
      offlineArtist: "Cannot connect to server",
      liveTitle: "Live",
      liveArtist: "Radio Vixis",
      waiting: "Waiting for transmission...",
      unknownTitle: "Unknown title",
      unknownArtist: "Unknown artist",
      chat: "Chat",
      events: "Upcoming Events",
      noEvents: "No upcoming events",
      loadingEvents: "Loading events...",
      chatPlaceholder: "Type a message...",
      send: "Send",
      username: "Username",
      setUsername: "Set username",
      usernameRequired: "Please enter a username",
      messageTooShort: "Message cannot be empty",
      messageTooLong: "Message cannot exceed 200 characters",
      rateLimit: "Please wait before sending another message",
      sending: "Sending...",
      noMessages: "No messages yet",
      loadingMessages: "Loading messages...",
      autoplayBlocked: "Could not autoplay (may require user interaction)",
      ago: "ago",
      minutes: "minutes",
      hours: "hours",
      days: "days",
      now: "now",
      usernameTooShort: "The username must be at least 2 characters",
      usernameInvalid:
        "The username can only contain letters, numbers, hyphens and underscores",
      bannedWords: "The message contains banned words",
      requestSong: "Request a Song",
      songRequestUrlNotConfigured: "Typeform URL not configured",
      jingleTitle: "Tune in!",
      connecting: "Connecting...",
    },
    // Blog
    blog: {
      title: "Blog",
      loading: "Loading posts...",
      noPosts: "No posts available",
      noPostsDescription: "Come back soon to see my posts",
      readMore: "Read more",
      platform: "Platform",
      date: "Date",
    },
    // Studio
    studio: {
      title: "Vixis Studio",
      description: "We create systems that think and art that connects.",
      mission: "Mission",
      vision: "Vision",
      missionDescription:
        "Our mission is to create innovative and efficient solutions for our payers.",
      visionDescription:
        "Our vision is to be a reference and a development agency that creates art that connects with communities through automation.",
      technologies: "Technologies:",
      status: "Status:",
      client: "Payer:",
      active: "In development",
      comingSoon: "Coming soon...",
      activeStatus: "Status: Active",
      highPriority: "Priority: High",
      socialNetworks: "Social Networks",
      socialNetworksSubtitle: "Connect with us on your favorite platforms",
      instagramDesc: "Visual content and creative projects",
      tiktokDesc: "Short videos about development and tech",
      whatsappDesc: "Direct contact and support",
      facebookDesc: "Community and updates",
      telegramDesc: "Official channel and announcements",
      xDesc: "Quick updates and conversations",
    },
    // Public contracts
    contracts: {
      title: "Service agreement",
      lockedDesc: "This contract is protected. Enter the password you received with the link to view and sign it.",
      passwordPh: "Contract password",
      unlock: "Unlock contract",
      unlocking: "Verifying…",
      client: "Client",
      company: "Company",
      amount: "Amount",
      pendingSig: "Awaiting full signature",
      signedBoth: "Completed — signed by both parties",
      signatures: "Signatures",
      provider: "Provider",
      signed: "Signed",
      notSigned: "Not signed",
      signNote: "By signing you accept the terms above. The signature is electronic (name + date/time) and email-verified.",
      namePh: "Your full name",
      emailPh: "Your email (must match the contract)",
      signBtn: "Sign contract",
      signing: "Signing…",
      otpTitle: "Email verification",
      otpDesc: "We sent a 6-digit code to",
      sendCode: "Send code",
      sendingCode: "Sending…",
      newCode: "Request another code",
      codePh: "6-digit code",
      verify: "Verify",
      verifying: "Verifying…",
      verified: "Verified ✓",
      download: "Download contract PDF",
      generating: "Generating PDF…",
      needName: "Type your name to sign",
    },
    // Status
    status: {
      title: "Professional Status",
      sendRequest: "Send a Request",
      hoursByStatus: "Hours by Status (Week)",
      availableHours: "Available Hours (Week)",
      statusDistribution: "Status Distribution (Total)",
      available: "Available",
      away: "Away",
      busy: "Busy",
      availableThisWeek: "Available this week",
      awayThisWeek: "Away this week",
      busyThisWeek: "Busy this week",
      requestForm: "Send Request",
      formDescription: "Complete the form to send your request",
      name: "Name",
      namePlaceholder: "Your full name",
      email: "Email",
      emailPlaceholder: "your.email@example.com",
      type: "Request Type",
      typePlaceholder: "Select type",
      jobOffer: "Job Offer",
      collaboration: "Collaboration",
      project: "Project",
      other: "Other",
      message: "Project Brief / Message",
      messagePlaceholder: "Describe your vision...",
      estimatedInvestment: "Estimated Investment",
      currency: "Currency",
      currencyCOP: "COP",
      currencyUSD: "USD",
      selectCurrency: "Select currency",
      selectRange: "Select range",
      encryptionNotice:
        "ACTIVE ENCRYPTION. YOUR DATA WILL BE PROCESSED UNDER A+ LEVEL PRIVACY PROTOCOLS.",
      successMessage:
        "Your message was sent successfully. I'll get in touch with you soon.",
      giveTestimonial: "Give a Testimonial",
      testimonialUrlNotConfigured: "Typeform URL not configured",
      submit: "Submit",
      sendProposal: "Send Proposal",
      cancel: "Cancel",
      visionTitle: "WORK WITH ME",
      visionDescription:
        "Do you have a challenging project, a technical question, or simply want to expand your network? Let's talk.",
      step1Title: "INITIAL CONTACT",
      step1Description:
        "The contact will be redirected to my email and I will analyze your message in a few hours.",
      step2Title: "DISCOVERY CALL",
      step2Description:
        "We schedule a meeting of approximately 15 minutes to align objectives and vision.",
      step3Title: "TECHNICAL PROPOSAL",
      step3Description:
        "I send you a detailed plan, budget, and real timeline.",
      directEmail: "DIRECT EMAIL",
      callWhatsapp: "DIRECT WHATSAPP",
      preferVideocall: "DO YOU PREFER A VIDEO CALL?",
      discoveryCall: "15 MIN DISCOVERY CALL",
      scheduleNow: "SCHEDULE NOW",
      orWriteHere: "OR WRITE ME HERE",
      identityName: "IDENTITY / NAME",
      linkChannelEmail: "LINK CHANNEL / EMAIL",
      mobileFrequencyPhone: "MOBILE FREQUENCY / PHONE",
      pesos: "PESOS",
      dollars: "DOLLAR",
      example: "Request example:",
      exampleName: "Name: Carlos Vicioso",
      exampleEmail: "Email: your.email@example.com",
      exampleType: "Type: Job Offer",
      exampleMessage:
        "Message: Hello, I would like to offer you a backend developer position in our company. The project involves modern technologies and a dynamic team. Would you be interested in learning more details?",
      exampleNote:
        "Upon sending, you will receive a confirmation and I will contact you soon to discuss the details.",
      monday: "Monday",
      tuesday: "Tuesday",
      wednesday: "Wednesday",
      thursday: "Thursday",
      friday: "Friday",
      mon: "Mon",
      tue: "Tue",
      wed: "Wed",
      thu: "Thu",
      fri: "Fri",
      success: "Request sent successfully",
      error: "Error sending request",
    },
    // NotFound
    notFound: {
      gameOver: "Game Over",
      fileNotFound: "404 FILE NOT FOUND",
      backToHome: "Back to Home",
    },
    // Admin
    admin: {
      title: "Admin Panel",
      restrictedAccess: "Restricted Access",
      login: "Login",
      username: "Username",
      password: "Password",
      logout: "Logout",
      invalidCredentials: "Invalid username or password",
      contentManagement: "Content Management",
      groups: {
        dashboard: "Overview",
        content: "Content",
        general: "General",
        system: "System",
      },
      overview: "Overview",
      viewSite: "View Site",
      externalTools: "External Tools",
      tools: {
        ga4: "Google Analytics 4",
        ga4Desc: "Portfolio traffic analytics; feeds the BOS through its free Data API.",
        console: "Google Search Console",
        consoleDesc: "Site performance and index in Google.",
        bing: "Bing Webmasters",
        bingDesc: "SEO and performance of vixis.dev on Bing.",
        betterstack: "BetterStack",
        betterstackDesc: "Uptime and logs for production services.",
      },
      blogPosts: "Blog Posts",
      homeContent: "Home Content",
      radioSettings: "Radio Settings",
      invoices: "Invoices",
      appearance: "Appearance",
      appearanceContractsBg: "Contracts background",
      appearanceLegalBg: "Legal pages background (Privacy & Terms)",
      appearanceDefault: "Default",
      appearanceStarry: "Starry Night (Animated Canvas)",
      appearanceBrand: "Brand (emails & documents)",
      brandName: "Brand name",
      brandLogo: "Logo (URL)",
      brandColor: "Primary color (buttons)",
      brandFooter: "Email footer",
      jobOffers: "Job Offers",
      crm: {
        label: "CRM",
        subtitle: "Contacts, companies, sales pipeline, activities, leads, emails and contracts",
        loading: "Loading CRM…",
        error: "Error:",
        tabs: {
          contacts: "Contacts",
          companies: "Companies",
          pipeline: "Pipeline",
          activities: "Activities",
          leads: "Leads & visits",
          emails: "Emails",
          contracts: "Contracts",
          integrations: "Integrations",
        },
        newCompany: "+ Company",
        newContact: "+ Contact",
        newDeal: "+ Deal",
        newContract: "+ Contract",
        save: "Save",
        saving: "Saving…",
        cancel: "Cancel",
        edit: "Edit",
        delete: "Delete",
        send: "Send",
        sending: "Sending…",
        close: "Close",
        exportCsv: "Export CSV",
        importCsv: "Import CSV",
        importing: "Importing…",
        noDataExport: "No data to export",
        emptyCsv: "Empty CSV or missing header",
        missingColumn: "Missing column",
        contact: "Contact",
        contacts: "contacts",
        company: "Company",
        companies: "companies",
        imported: "Imported",
        importedF: "Imported",
        createdCompanies: "companies created",
        rowsSkipped: "rows without name skipped",
        linkCopied: "Link copied",
        sentOk: "Email sent successfully",
        errSaveContact: "Error saving contact",
        errSaveCompany: "Error saving company",
        errSaveDeal: "Error saving deal",
        errMoveDeal: "Error moving deal",
        errSaveActivity: "Error saving activity",
        errUpdateActivity: "Error updating activity",
        errConvertLead: "Error converting lead",
        errSaveTemplate: "Error saving template",
        errDelete: "Error deleting",
        errSendEmail: "Error sending email",
        errSaveContract: "Error saving contract",
        errSign: "Error signing",
        errRegen: "Error regenerating",
        errImport: "Error importing CSV",
        mapTitle: "Review columns",
        mapHint:
          "We detected your file columns automatically. Adjust as needed; fields with no column are left empty.",
        mapIgnore: "Ignore",
        mapSourceCol: "File column",
        mapImport: "Import",
        mapUnmatched: "Unused columns",
        mapDetected: "detected",
        duplicatesSkipped: "duplicates skipped",
        vcardDetected: "vCard file detected",
        noUsableColumns:
          "We couldn't recognize any column. Check the file or adjust the mapping manually.",
        fieldFirstName: "First name",
        fieldLastName: "Last name",
        fieldFullName: "Full name",
        fieldEmail: "Email",
        fieldPhone: "Phone",
        fieldPhone2: "Phone 2",
        fieldCompany: "Company",
        fieldJobTitle: "Job title",
        fieldLinkedin: "LinkedIn",
        fieldGithub: "GitHub",
        fieldXHandle: "X / Twitter",
        fieldWebsite: "Website",
        fieldAddress: "Address",
        fieldBirthdate: "Birthday",
        fieldGender: "Gender",
        fieldExperience: "Years of experience",
        fieldOwner: "Owner",
        fieldTags: "Tags",
        fieldSource: "Source",
        fieldNotes: "Notes",
        fieldDomain: "Domain",
        fieldIndustry: "Industry",
        fieldName: "Name",
        fieldCompanyName: "Name",
        confirmDeleteTemplate: "Delete template? Past sends are kept.",
        confirmDeleteContract: "Delete contract? The public link will stop working immediately. This cannot be undone.",
        confirmDeleteContact: "Delete contact? Their activities will be removed and deals left unassigned.",
        confirmDeleteCompany: "Delete company? Its contacts and deals will be left without company.",
        confirmDeleteDeal: "Delete deal? Its activities will be removed.",
        confirmDeleteActivity: "Delete activity?",
        confirmRegenPwd: "Regenerate the contract password. The previous one will stop working. Continue?",
        signPrompt: "Provider signer name:",
        contactsTab: {
          searchPh: "Search by name, email, phone or tag…",
          thContact: "Contact",
          thCompany: "Company",
          thEmailPhone: "Email / Phone",
          thTags: "Tags",
          thOrigin: "Source",
          noEmail: "No email",
          noPhone: "No phone",
          sortBy: "Sort",
          sortName: "Name A–Z",
          sortCompany: "Company",
          sortRecent: "Newest",
          sortOldest: "Oldest",
          filterSource: "Source",
          allSources: "All",
        },
        tabContact: "Contact",
        tabCompany: "Company",
        grid: {
          search: "Search {{entity}}…",
          fields: "Fields",
          new: "+ {{entity}}",
          open: "Open",
          deleteRow: "Delete",
          clickToEdit: "click to edit",
          addField: "Add field",
          sortAsc: "Sort ▲",
          sortDesc: "Sort ▼",
          hideColumn: "Hide column",
          save: "Save",
          cancel: "Cancel",
          required: "{{label}} (required)",
          creatingLabel: "Creating {{entity}}…",
          empty: "No {{entity}}s yet.",
          noMatches: "No results for this search.",
          count: "{{n}} records",
          deleteSelected: "Delete ({{n}})",
          deleteSelectedConfirm: "Delete",
          bulkEdit: "Edit ({{n}})",
          bulkTitle: "Bulk edit ({{n}})",
          bulkField: "Field",
          bulkApply: "Apply to {{n}}",
          bulkAdd: "Add",
          bulkRemove: "Remove",
          clearSelection: "Clear selection",
          selectAll: "Select all",
          selectRow: "Select row",
          rowMenu: "Row options",
          saveError: "Could not save. Changes were reverted.",
          createError: "Could not create the record.",
        },
        drawer: {
          title: "Details",
          relationship: "Relationships",
          delete: "Delete",
          close: "Close",
        },
        fields: {
          title: "{{entity}} fields",
          add: "Add field",
          close: "Close",
          labelField: "Field name",
          labelPh: "E.g. Social networks",
          optionsPh: "Comma-separated options (select/multi only)",
          nameRequired: "Type a name for the field.",
          nameTaken: "A field with that name already exists.",
          saveField: "Save field",
          toggle: "Show / hide",
          system: "system",
          readonly: "read-only",
          hide: "Hide",
          show: "Show",
          showAll: "Show all",
          hideAll: "Hide all",
          visibleOf: "{{v}} of {{n}} visible",
          delete: "Delete",
          confirmDelete: "Delete this field? Stored values will be removed.",
          error: "Could not save the field.",
        },
        contactModal: {
          new: "New contact",
          edit: "Edit contact",
          firstName: "First name *",
          firstNamePh: "First name",
          lastName: "Last name",
          lastNamePh: "Last name",
          email: "Email",
          emailPh: "name@domain.com",
          phone: "Phone",
          phonePh: "+1 …",
          company: "Company",
          noCompany: "No company",
          origin: "Source",
          srcManual: "Manual",
          srcTally: "Tally",
          srcWhatsapp: "WhatsApp",
          srcPortfolio: "Portfolio",
          jobTitle: "Job title",
          jobTitlePh: "E.g.: CTO, Designer…",
          phone2: "Phone 2",
          birthdate: "Birthdate",
          gender: "Gender",
          genderUnspecified: "Unspecified",
          genderFemale: "Female",
          genderMale: "Male",
          genderOther: "Other",
          linkedin: "LinkedIn",
          github: "GitHub",
          xHandle: "X (Twitter)",
          website: "Website",
          address: "Address",
          experienceYears: "Yrs exp.",
          owner: "Owner",
          tags: "Tags (comma separated)",
          tagsPh: "lead, client…",
          notes: "Notes",
          notesPh: "Context, preferences…",
        },
        contactDetail: {
          noEmail: "No email",
          noPhone: "No phone",
          deals: "Deals",
          noDeals: "No linked deals.",
          activities: "Activities",
          noActivities: "No activities.",
          addActivity: "+ Activity",
          sendEmail: "Send email",
          memberSince: "In CRM since",
          profile: "Profile",
          social: "Social & web",
          extra: "Additional data",
        },
        companiesTab: {
          count: "companies",
          thCompany: "Company",
          thIndustry: "Industry",
          thContacts: "Contacts",
          thDeals: "Deals",
          thDomain: "Domain",
          empty: "No companies yet. Create one with \"+ Company\" (the logo is fetched from the domain automatically).",
          sortBy: "Sort",
          sortName: "Name A–Z",
          sortContacts: "Most contacts",
        },
        companyModal: {
          new: "New company",
          edit: "Edit company",
          name: "Name *",
          namePh: "Company name",
          domain: "Domain (fetches the logo)",
          domainPh: "domain.com",
          industry: "Industry",
          industryPh: "E.g.: SaaS, Development…",
          notes: "Notes",
          notesPh: "Details/context",
          logoNote: "Current logo (auto-updates when the domain changes)",
          tags: "Tags (comma separated)",
          tagsPh: "client, partner…",
          foundedYear: "Founded",
          employeeRange: "Employees",
          empRange1: "1–10",
          empRange2: "11–50",
          empRange3: "51–200",
          empRange4: "201–1000",
          empRange5: "1000+",
          nit: "Tax ID",
          nitPh: "900123456-7",
          address: "Address",
          phone: "Phone",
          linkedin: "LinkedIn",
          github: "GitHub",
          xHandle: "X (Twitter)",
          website: "Website",
          owner: "Owner",
        },
        pipeline: {
          newHere: "New deal in this stage",
          moveTo: "Move to",
          close: "Close:",
          noDeals: "No deals",
          noStages: "No stages configured.",
        },
        dealModal: {
          new: "New deal",
          edit: "Edit deal",
          title: "Title *",
          titlePh: "E.g.: Corporate website",
          contact: "Contact",
          noContact: "No contact",
          company: "Company",
          noCompany: "No company",
          stage: "Stage",
          value: "Value",
          currency: "Currency",
          prob: "Prob. %",
          closeDate: "Expected close",
        },
        activitiesTab: {
          allPeople: "Everyone",
          addActivity: "+ Activity",
          thType: "Type",
          thSubject: "Subject",
          thPerson: "Person",
          thDue: "Due date",
          thDone: "Done",
          empty: "No activities yet. Create one with \"+ Activity\".",
        },
        activityModal: {
          new: "New activity",
          edit: "Edit activity",
          type: "Type",
          typeNote: "Note",
          typeEmail: "Email",
          typeCall: "Call",
          typeTask: "Task",
          typeMeeting: "Meeting",
          due: "Due date",
          subject: "Subject *",
          subjectPh: "Subject",
          person: "Person",
          noPerson: "No person",
          deal: "Deal",
          noDeal: "No deal",
          detail: "Details",
          detailPh: "Notes / follow-up",
        },
        leadsTab: {
          title: "Leads",
          converted: "converted",
          pending: "pending",
          noName: "Lead without data",
          toContact: "+ Contact",
          converting: "Converting…",
          inCrm: "In CRM",
          noLeads: "No leads yet",
          visitorsTitle: "Portfolio visitors",
          today: "today",
          total: "total",
          noVisits: "No visits recorded",
          dedupeNote: "One visit per session + page + day (automatic dedupe).",
          from: "from",
          country: "Country",
          device: "Device",
          page: "Page",
          campaign: "Campaign",
        },
        contractTypes: {
          servicios: "Services",
          consultoria: "Consulting",
          nda: "Non-Disclosure (NDA)",
          oferta: "Commercial Offer",
          soporte: "Support & Maintenance",
          licencia: "Licensing",
          otro: "Other",
        },
        emailsTab: {
          hint: "Templates with placeholders —",
          newTemplate: "+ Template",
          templates: "Templates",
          active: "active",
          paused: "paused",
          noTemplates: "No templates. Create one to send emails.",
          recent: "Recent sends",
          noSends: "No sends yet.",
          noSubject: "no subject",
          sent: "Sent",
          pending: "Pending",
          failed: "Error",
        },
        templateModal: {
          new: "New template",
          edit: "Edit template",
          name: "Name *",
          namePh: "Initial offer",
          status: "Status",
          subject: "Subject *",
          subjectEn: "English subject (optional)",
          body: "Body (plain text, with placeholders)",
          bodyEn: "English body (optional)",
          placeholdersNote: "Available placeholders:",
        },
        sendModal: {
          title: "Send email",
          contact: "Contact *",
          select: "Select…",
          noEmailOpt: "no email",
          template: "Template *",
          pausedOpt: "[paused] ",
          deal: "Deal (optional)",
          noDeal: "No deal",
          lang: "Language",
          preview: "Preview (with contact data):",
          noEmailWarn: "The selected contact has no email. Add one before sending.",
        },
        contractsTab: {
          desc: "Contracts with password-protected public link. Once both parties sign it can be downloaded as PDF.",
          pwdLabel: "Access password:",
          copiedClose: "Copied — close",
          thContract: "Contract",
          thClient: "Client",
          thAmount: "Amount",
          thStatus: "Status",
          thLink: "Link",
          thActions: "Actions",
          created: "Created",
          signed: "Signed ✓",
          partial: "Partial signature",
          sent: "Sent",
          copyLink: "Copy link",
          sign: "Sign",
          signedBtn: "Signed",
          key: "Key",
          noContracts: "No contracts.",
          failedTip: "failed attempts (24h)",
        },
        integrationsTab: {
          desc: "Connect Google Contacts or import vCard/CSV files. Deduplication prevents creating repeated contacts.",
          notConnected: "Not connected",
          connect: "Connect with Google",
          sync: "Sync contacts",
          working: "Working…",
          disconnect: "Disconnect",
          status: "Status",
          lastSync: "Last sync",
          imported: "New",
          linked: "linked",
          skipped: "skipped",
          syncDone: "Sync completed",
          confirmDisconnect: "Disconnect Google Contacts from the CRM?",
          notConfigured: "GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET are missing from Supabase secrets.",
          err: "Integration error",
          fileTitle: "Import files",
          fileDesc: "vCard (.vcf) from Google/Outlook or CSV with any column order.",
          importContacts: "Import contacts",
          importCompanies: "Import companies",
          fileFormats: "Formats: .vcf, .vcard, .csv",
        },
        contractModal: {
          new: "New contract",
          edit: "Edit contract",
          title: "Title *",
          titlePh: "Website project — initial scope",
          titleEn: "English title (optional)",
          ctype: "Contract type",
          contact: "Contact",
          noContact: "No contact",
          company: "Company",
          noCompany: "No company",
          currency: "Currency",
          amount: "Amount",
          terms: "Terms *",
          termsPh: "1. Scope\n2. Timeline\n3. Payments\n…",
          termsEn: "English terms (optional)",
          editNote: "Editing does not change the current password. Signing is managed from the table.",
          createNote: "Creating the contract auto-generates the password and the public link.",
        },
      },
      bos: {
        subtitle: "Business-wide performance",
        revenue30: "Revenue 30 days",
        revenue90: "Revenue 90 days",
        lifetime: "Lifetime",
        ticket: "Avg ticket 30d",
        orders: "orders",
        revenueChart: "Revenue — last 30 days",
        trafficChart: "Web traffic — 30 days",
        recentSales: "Recent sales",
        noSales: "No paid sales yet",
        byGateway: "Revenue by gateway (90d)",
        noData: "No data",
        leads: "Leads",
        total: "total",
        noLeads: "No leads yet",
        converted: "converted",
        pending: "pending",
        toCrm: "→ CRM",
        inCrm: "In CRM",
        noName: "Lead without data",
        visitors: "Portfolio visitors",
        today: "today",
        totalVisitors: "total recorded",
        noVisits: "No visits recorded yet",
        connectors: "Connectors",
      },
      mediaManagement: "Media Resources Management",
      show: "Show",
      hide: "Hide",
      mediaResources: "Media resources",
      changeStatus: "Change Status",
      products: "Products",
      projects: "Projects",
      clients: "Payers",
      testimonials: "Testimonials",
      socials: "Social Media",
      events: "Events",
      workExperiences: "Work Experience",
      technologies: "Technologies",
      studies: "Studies",
      noItems: "No items available",
      createNew: "Create new",
      editItem: "Edit item",
      deleteItem: "Delete item",
      saveSuccess: "Item saved successfully",
      deleteSuccess: "Item deleted successfully",
      error: "Error processing request",
      noProducts: "No products available",
      noProjects: "No projects available",
      noClients: "No clients available",
      noTestimonials: "No testimonials available",
      noSocials: "No social media available",
      noEvents: "No events available",
      noWorkExperiences: "No work experiences available",
      noTechnologies: "No technologies available",
      noStudies: "No studies available",
      noInvoices: "No invoices available",
      createNewToStart: "Create a new one to get started.",
      radioControl: "Radio Control",
      icecastStatus: "Icecast Status",
      liquidsoapStatus: "Liquidsoap Status",
      cloudflareTunnelStatus: "Cloudflare Tunnel Status",
      radioFrontend: "Radio Frontend",
      cloudflareDashboard: "Cloudflare Dashboard",
      active: "Active",
      inactive: "Inactive",
      unknown: "Unknown",
      // Modal titles
      editProduct: "Edit Product",
      createProduct: "Create Product",
      editProject: "Edit Project",
      createProject: "Create Project",
      editClient: "Edit Payer",
      createClient: "Create Payer",
      editTestimonial: "Edit Testimonial",
      createTestimonial: "Create Testimonial",
      editSocial: "Edit Social Media",
      createSocial: "Create Social Media",
      editEvent: "Edit Event",
      createEvent: "Create Event",
      editWorkExperience: "Edit Work Experience",
      createWorkExperience: "Create Work Experience",
      editTechnology: "Edit Technology",
      createTechnology: "Create Technology",
      editStudy: "Edit Study",
      createStudy: "Create Study",
      // Form fields
      fieldTitle: "Title",
      fieldDescription: "Description",
      fieldFullDescription: "Full Description",
      fieldBasePrice: "Base Price (COP)",
      fieldThumbnailUrl: "Thumbnail URL",
      fieldActionType: "Action Type",
      fieldActionUrl: "Action URL",
      fieldPricingLink: "Pricing Link",
      fieldButtonText: "Button Text",
      fieldUrl: "URL",
      fieldRepository: "Repository",
      fieldMonth: "Month",
      fieldYear: "Year",
      fieldName: "Name",
      fieldLogo: "Logo URL",
      fieldTestimonialContent: "Testimonial Content",
      fieldTestimonialAuthor: "Testimonial Author",
      fieldTestimonialRole: "Author Role",
      fieldTestimonialUrl: "Testimonial URL",
      fieldIcon: "Icon URL",
      fieldPosition: "Position",
      fieldCompany: "Company",
      fieldCompanyUrl: "Company URL",
      fieldCompanyLogo: "Company Logo URL",
      fieldLocation: "Location",
      fieldStartDate: "Start Date",
      fieldEndDate: "End Date",
      leaveEmptyIfCurrent: "Leave empty if current job",
      fieldResponsibilities: "Responsibilities (JSON Array)",
      fieldTechnologies: "Technologies (JSON Array)",
      fieldType: "Type",
      fieldTypeOptionCourse: "Course",
      fieldTypeOptionCertification: "Certification",
      fieldTypeOptionDegree: "Degree",
      fieldStatus: "Status",
      completed: "Completed",
      inProgress: "In Progress",
      fieldInstitution: "Institution",
      fieldCertificateUrl: "Certificate URL",
      fieldYearsOfExperience: "Years of Experience",
      fieldCategory: "Category",
      fieldLevel: "Level",
      // Form options
      actionTypeLink: "Link",
      actionTypeSubmit: "Submit",
      actionTypeSchedule: "Schedule",
      // Media Manager
      resourceType: "Resource Type",
      selectFile: "Select File",
      optimizeAndUpload: "Optimize and Upload",
      uploading: "Uploading...",
      uploadSuccess: "File uploaded successfully! URL copied to clipboard.",
      uploadError: "Error uploading file",
      urlCopied: "URL copied to clipboard",
      copyUrl: "Copy URL",
      // Resource options
      productThumbnails: "Product Thumbnails",
      productImages: "Product Images",
      eventThumbnails: "Event Thumbnails",
      generalAssets: "General Assets",
      // Messages
      confirmDelete: "Are you sure you want to delete this item?",
      deleteWarning: "This action cannot be undone.",
      errorDeleting: "Error deleting",
      errorSaving: "Error saving",
      errorUnknown: "Unknown error",
      errorGeneratingLink: "Error generating pricing link",
      errorExtractingData: "Error extracting data",
      pleaseEnterUrl: "Please enter a valid URL",
      generateLink: "Generate",
      extractEventData: "Extract Event Data",
      eventUrl: "Event URL",
      eventDate: "Event Date",
      supportsEventPlatforms:
        "Supports Passline, start.gg, duel.plus and other platforms",
      clientCannotChange: "Payer cannot be changed when editing",
      buttonTextPlaceholder: "E.g: Buy, Schedule, etc.",
      projectStatus: "Project Status",
      noDate: "No date",
      noDescription: "No description",
      noTestimonial: "No testimonial",
      noClient: "Payer",
    },
    // Status Badge
    statusBadge: {
      confirm:
        "Are you sure you want to continue? I may not respond to you due to various reasons...",
      available: "Available for new projects",
      away: "Away",
      busy: "Not available",
    },
    // Contact Section
    contactSection: {
      title: "Contact",
      description:
        "Creating unique digital experiences. Let's talk about your next project!",
      sendRequest: "Send your proposal",
      contact: "Contact",
    },
    // Home Section
    homeSection: {
      projects: "Projects",
    },
    // Latest Post Card
    latestPostCard: {
      title: "Latest Post",
      readMore: "Read more",
    },
    // Legal
    legal: {
      privacyTitle: "Privacy Policy",
      termsTitle: "Terms of Service",
      privacyLink: "Privacy Policy",
      termsLink: "Terms of Service",
    },
    // How to Pay Me
    howToPay: {
      title: "How to Pay Me?",
      description: "Follow these simple steps to complete your payment:",
      payButton: "Pay now",
      step1: "Make sure to include the product ID and invoice number in the payment note.",
      step2: "Your payment will be processed within 24-48 hours after confirmation.",
      step3: "If you have any questions, please contact me through my social media.",
      poweredBy: "Secure payments powered by",
      paymentMethods: "+500 payment methods",
      paymentDetails: "Bank transfers, crypto, mobile payments & more. Cancel anytime.",
    },
    // Custom Checkout
    checkout: {
      title: "Checkout",
      backToProduct: "Back to product",
      backToStore: "Back to store",
      productNotFound: "Product not found",
      invoiceNotFound: "Invoice not found",
      invoiceNotFoundHint: "The link is invalid or the invoice no longer exists.",
      goHome: "Go home",
      error: "Error",
      buyerInfo: "Buyer information",
      fullName: "Full name",
      email: "Email",
      deliveryNote:
        "You'll receive your invoice and product access at this email.",
      nameRequired: "Your name is required",
      emailRequired: "Your email is required",
      emailInvalid: "Invalid email",
      tabCard: "Card",
      tabCrypto: "Cryptocurrencies",
      payWithCard: "Pay with card (Visa, Mastercard, Amex, PSE)",
      cardInfo:
        "Secure payment processed by dLocal. Credit, debit cards and local methods.",
      payCardButton: "Pay with card",
      confirmCardButton: "Confirm payment",
      documentType: "Document type",
      documentNumber: "Document number",
      documentRequired: "Document number is required",
      country: "Country",
      countryRequired: "Select your country",
      selectCountry: "Select your country",
      cardTokenError: "The card could not be processed, please check the details",
      cardUnavailable: "Card payments are not available right now.",
      secureNote:
        "Your card details are processed securely by dLocal (PCI DSS compliant).",
      payWithCrypto: "Pay with cryptocurrencies",
      cryptoInfo:
        "I accept BTC, ETH, USDT and 300+ cryptocurrencies. You'll get a secure payment link.",
      payCryptoButton: "Pay with cryptocurrencies",
      paypalUnavailable: "PayPal is not available right now.",
      paypalError: "There was a PayPal error. Please try again.",
      creatingPayment: "Creating secure payment...",
      waitingConfirmation: "Waiting for payment confirmation...",
      confirmingPayment: "Confirming your payment...",
      stillWaiting:
        "Your payment is being processed. We'll email you your access.",
      paymentFailed: "The payment could not be completed.",
      noRedirect: "Could not get the payment link",
      successTitle: "Payment confirmed!",
      successMessage:
        "Payment confirmed! I've emailed your invoice and product access.",
      invoiceLabel: "Invoice",
      yourAccess: "Access your product:",
      done: "Done",
    },
  },
} as const;

// Hook para usar traducciones
export function useTranslation() {
  const { language } = useLanguageStore();
  const t = (key: string) => {
    const keys = key.split(".");
    let value: any = translations[language];
    for (const k of keys) {
      value = value?.[k];
    }
    return value || key;
  };
  return { t, language };
}

// Función helper para obtener traducción directa
export function getTranslation(key: string, lang?: "es" | "en"): string {
  const language = lang || useLanguageStore.getState().language;
  const keys = key.split(".");
  let value: any = translations[language];
  for (const k of keys) {
    value = value?.[k];
  }
  return value || key;
}

// Tipo para campos traducibles (JSONB en Supabase)
export type TranslatableField =
  | {
      es?: string;
      en?: string;
    }
  | string
  | null;

/**
 * Obtiene el texto traducido de un campo que puede ser:
 * - Un objeto con claves 'es' y 'en' (JSONB de Supabase)
 * - Un string simple (fallback para datos antiguos)
 * - null/undefined
 */
export function getTranslatedText(
  field: TranslatableField,
  lang?: "es" | "en"
): string {
  const language = lang || useLanguageStore.getState().language;

  if (!field) return "";

  // Si es un string simple, devolverlo directamente
  if (typeof field === "string") {
    return field;
  }

  // Si es un objeto con traducciones
  if (typeof field === "object" && field !== null) {
    // Intentar obtener la traducción del idioma actual
    const translated = field[language];
    if (translated) return translated;

    // Fallback al otro idioma si existe
    const fallbackLang = language === "es" ? "en" : "es";
    if (field[fallbackLang]) return field[fallbackLang];

    // Si no hay traducción, devolver el primer valor disponible
    const firstValue = field.es || field.en;
    if (firstValue) return firstValue;
  }

  return "";
}

/**
 * Helper para obtener múltiples campos traducidos de un objeto
 */
export function getTranslatedFields<T extends Record<string, any>>(
  item: T,
  fields: (keyof T)[],
  lang?: "es" | "en"
): Partial<Record<keyof T, string>> {
  const result: Partial<Record<keyof T, string>> = {};
  for (const field of fields) {
    result[field] = getTranslatedText(item[field] as TranslatableField, lang);
  }
  return result;
}
